...
app.get("/todos/", (req, res, next) => {
    ...
    // Get elements from the Query String
    ...
    // Assign the User Name to each Todo
    filteredTodos.map(todo => {
        todo.userName = users.find(user => user.id == todo.userId).name
    })

    // Data that is being sent into the index page
    let data = {
        todos: filteredTodos,
        users: users
    }
...
app.post("/todos", (req, res, next) => {
    ...
    // Add the information from the new Todo
    let newTodo = {
        id: req.body.id,
    ...
    // #1 Assign an ID to the new Todo
    // The easy way - work with the Todo Array length

    // #2 (optional) Display an error if a user has already that Todo
    if (users.find(user => {
        
    }))
    {
        // if you find a given userid with the title
        // put data.error = "User already has that Todo"
        // and don't insert that Todo.
    }
    else {
        // Insert the new Todo and to the logic below
    }
    ...
    // Now we're going to retrieve again all the information
    let data = {
        todos: todos,
        users: users,
        error: "",
    ...
    // Assign names to the Todos list
    data.todos.map(todo => {
        todo.userName = users.find(user => user.id == todo.userId).name
    })

    // Assign a Success Message to the Page
    data.success = "Todo Inserted !!!"
